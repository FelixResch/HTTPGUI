...
public class ActionManager {
	...
	public Updates invokeAction(String target, Browsers browser) {
		...
		if (args.length == cmdArgs.length) {
			updates.addUpdate("messages", HTMLCompiler.compileObject(new Warning(
		...
		for (int i = 0; i < args.length; i++) {
			if (!args[i].split("=")[0].equals(cmdArgs[0])) {
				updates.addUpdate("messages", HTMLCompiler.compileObject(
						new Warning("Args order is wrong"), browser));
				return updates;
			}
		}
		...
		try {
			...
			updates.addUpdate(
					"messages",
					HTMLCompiler.compileObject(new Warning("Some error happened:"
